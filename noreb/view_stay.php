...
<div id='content'>
<h3>Stays displayed four at-a-time</h3>
<p>
<?php
// This code fetches all the records from the houses table
require ('includes/config.php'); // Connect to the database
...
$q = "SELECT COUNT(ref_num) FROM stays";
...
// Make the query
$q = "SELECT ref_num, loctn, thumb, price, mini_descr, b_rooms, status FROM stays
...
echo "<p><center>Total found: $houses</center></p>";
if ($pages > 1) {
echo '<p>';
...
if ($current_page != 1) {
echo '<center><a href="view_stay.php?s=' . ($start - $pagerows) . '&p=' . $pages .'">Previous</a> </center>';
}
//Create a Next link
if ($current_page != $pages) {
echo'<center><a href="view_stay.php?s=' . ($start + $pagerows) . '&p=' . $pages . '">Next</a> </center>';
}
echo '</p>';
}
?>
